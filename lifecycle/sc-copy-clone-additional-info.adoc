...
* link:../bp/hana-aff-fc-introduction.html["ファイバチャネルプロトコルを使用したNetAppオールフラッシュFASシステム上のSAP HANA"]
* link:../bp/hana-aff-nfs-introduction.html["NFSを使用したNetAppオールフラッシュFASシステム上のSAP HANA"]
* link:../backup/fsxn-overview.html["Amazon FSX上のSAP HANA for NetApp ONTAP - SnapCenter を使用したバックアップとリカバリ"]
* link:lama-ansible-introduction.html["Ansibleを使用したNetApp SAP Landscape Managementの統合"]
* link:libelle-sc-overview.html["Libelle SystemCopyによるSAPシステムのコピー操作の自動化（netapp.com）"]
...
| バージョン4.1 | 2026年2月 | HANA暗号化とSAPローカルセキュアストアLSSに必要な手順を追加しました 
|===