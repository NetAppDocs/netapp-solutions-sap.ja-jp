...
SAPホストエージェントもこのホストにインストールされています。NetAppプロバイダスクリプトとAnsibleプレイブックは、Ansibleコントロールノードで設定しました（を参照）。link:lama-ansible-appendix-provider-script-configuration-and-ansible-playbooks.html["「付録：プロバイダスクリプトの設定」"]
