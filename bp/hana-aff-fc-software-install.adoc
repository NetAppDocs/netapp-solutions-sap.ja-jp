...
+
次の出力は、SID=FC5 の LVM を使用した 2+1 のマルチホスト セットアップの例を示しています。
...
....
+
SAP hdblcm インストール ツールを使用して、ワーカー ホストの 1 つで次のコマンドを実行してインストールを開始します。オプションを使用し `addhosts`て、2番目のワーカー（sapc-hana-tst-06）とスタンバイホスト（sapc-hana-tst-07）を追加します。

+

NOTE: 準備したファイルが格納されているディレクトリ `global.ini`は(`--storage_cfg=/hana/shared`、CLIオプションに含まれてい `storage_cfg`ます）。
