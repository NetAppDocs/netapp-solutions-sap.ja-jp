...
. セクションの説明に従って、アグリゲート内にボリュームとLUNを作成して構成します。 link:hana-aff-fc-host-setup_single_host.html["単一ホスト設定"]単一のホストまたはセクション内link:hana-aff-fc-host-setup_multiple_hosts.html["複数ホストのセットアップ"]
