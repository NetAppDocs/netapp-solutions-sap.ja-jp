...
NOTE: SAP HANAマルチホストシステムでは、 `/hana/shared`ファイルシステムと、SAP Local Secure Storeサービス（LSS）が使用されている場合は `/lss/shared`ファイルシステムが、システムのすべてのホストに接続されている必要があります。通常、これはNFSサーバによって提供されるNFS共有です。高可用性NFSサーバ（NetApp FASまたはAFFシステムなど）を使用することをお勧めします。別の選択肢として、LINUXホストの組み込みNFSサーバを使用することもできます。
