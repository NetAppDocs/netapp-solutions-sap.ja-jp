...
NOTE: Red Hat Enterprise Linuxバージョン8.6以降では、RHEL System Roles for SAP（Ansible）を使用して設定を適用することもできます。を参照してください https://launchpad.support.sap.com/["SAP ノート 3024346"^] （ SAP ログインが必要です）。
...
| rw 、 bg 、 hard 、 timeo = 600 、 noatime | バー = 3 、 nolock | vers=4 、 minorversion=1 、 lock です | rsize=1048576 、 wsize=262144 | rsize=65536 、 wsize=65536 です 
|===
...
NOTE: nconnect オプションは、 ONTAP 9.8 以降、ネットアップの NFSv4.1 でサポートされています。SLES15SP2 および RHEL 8.4 では、 NFSv4.1 を使用した nConnect をサポートする最初の NFS クライアントが使用できます。追加情報については、 Linux ベンダーのマニュアルを参照してください。
