...
`echo options nfs max_session_slots=<calculated value> > /etc/modprobe.d/nfsclient.conf`てから再起動します。サーバー側の値も調整する必要があります。で説明されているように、セッションスロットの数を設定します。 link:saphana_aff_nfs_storage_controller_setup.html#nfs-configuration-for-nfsv4["NFSv4 の NFS 設定"]
