...
以下の表は、2+1構成のマルチホストSAP HANAシステムのLUN構成を示しています。

|===
| 目的 | LUN | LVM論理ボリューム 


| データホスト1 | FC5_data_mnt00001_1 FC5_data_mnt00001_2 | FC5_data_mnt00001-vol 


| データホスト 2 | FC5_data_mnt00002_1 FC5_data_mnt00002_2 | FC5_data_mnt00002-vol 


| ログホスト1 | FC5_log_mnt00001_1 FC5_log_mnt00001_2 | FC5_log_mnt00001-vol 


| ログホスト2 | FC5_log_mnt00002_1 FC5_log_mnt00002_2 | FC5_log_mnt00002-vol 
|===

NOTE: /hana/sharedボリュームはすべてのSAP HANAホストからアクセス可能である必要があり、そのため通常はNFSを使用してエクスポートされます。NetApp ASAシステムはNFSをプロトコルとしてサポートしていないため、NetAppでは/hana/sharedファイルシステム用に追加のAFFまたはFASシステムを使用することを推奨しています。
...
NOTE: SAP Local Secure Storeサービス（LSS）を使用する場合、 `/lss/shared/SID`ディレクトリはすべてのホストからアクセス可能である必要があり、そのため通常はNFSを使用してエクスポートされます。NetApp ASAシステムはNFSをプロトコルとしてサポートしていないため、NetAppでは/lss/shared ファイルシステム用に追加のAFFまたはFASシステムを使用することを推奨しています。
...
NOTE: GUIを使用して、これらのLUNの自律型ランサムウェア対策を無効にします
...
. 必要な情報を入力してください:
+
.. LVMと結合するログLUNの名前。例： `FC5_log_mnt00001`
.. LVMと組み合わせるLUNの数、例：2
.. 各LUNのサイズ（例：260 GB）
.. 選ぶ `Linux`として `Host Operating system`
.. 以前に作成したマッピングを選択する `FC5_host`のために `Host mapping`オプション
...
=== すべてのデータおよびログ LUN に対して Autonomous Ransomware Protection を無効にする

各データLUNおよびログLUNのコンテキストメニューを開き、ランサムウェア対策を無効にします

image:saphana_asa_fc_image19_5.png["ONTAPシステムマネージャ - LUN の概要"]
