...
[role="lead"]
NetApp AFF AシリーズおよびAFF Cシリーズ製品ファミリーは、Tailored Data Center Integration（TDI；テーラードデータセンター統合）プロジェクトでSAP HANAとの使用が認定されています。このガイドでは、NFS 用のこのプラットフォーム上の SAP HANA のベスト プラクティスについて説明します。
