...
....
特定の LUN の最大スループットとして 1000 MB/秒を使用することをお勧めします。アプリケーションでより高いスループットが必要な場合は、LUN ストライピングを備えた複数の LUN を使用して必要な帯域幅を提供する必要があります。このガイドでは、Linux LVMベースのSAP HANAの例をセクションに示します。link:hana-asa-fc-host-setup.html["ホスト セットアップ"] 。
