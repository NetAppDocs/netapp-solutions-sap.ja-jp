...
[role="lead"]
SAP HANA のソフトウェアインストールの要件は次のとおりです。
...
. SAPインストールツールを使用して `hdbclm`、いずれかのワーカーホストで次のコマンドを実行してインストールを開始します。オプションを使用し `addhosts`て(`sapcc-hana-tst-03`、2番目のワーカーとスタンバイホストを追加し(`sapcc-hana-tst-04`ます）。
+
....
