...
ONTAP自律型ランサムウェア対策機能によって作成されたスナップショットは、SAP HANAデータベースのフォワードリカバリには使用できません。したがって、SAP HANAのデータボリュームおよびログボリュームには使用することは推奨されず、これらのボリュームでは無効にする必要があります。ランサムウェア対策として、改ざん防止スナップショットを用いたSnapCenterベースのデータ保護を使用してください：link:../backup/hana-sc-generic-scope.html["SnapCenter を使用した SAP HANA のバックアップとリカバリ"]。ONTAP自律型ランサムウェア対策は、/hana/sharedなどの他のボリュームには有効かもしれません。

ONTAP クラスタのボリューム作成時にこの機能の自動有効化を無効にすることは可能ですが、デフォルト設定のままにしておき、この機能が不要なボリュームでは無効にすることをお勧めします。
...
. の説明に従って、正しいSASスタック構成を確認します。link:hana-fas-nfs-storage-controller-setup.html#disk-shelf-connection["ディスクシェルフの接続"]
. の説明に従って、必要なアグリゲートを作成して設定します。link:hana-fas-nfs-storage-controller-setup.html#aggregate-configuration["アグリゲートの構成："]
. の説明に従ってStorage Virtual Machine（SVM）を作成します。link:hana-fas-nfs-storage-controller-setup.html#storage-virtual-machine-configuration["Storage Virtual Machine の設定。"]
. の説明に従ってLIFを作成します。link:hana-fas-nfs-storage-controller-setup.html#logical-interface-configuration["論理インターフェイスの構成"]
. およびの説明に従って、アグリゲート内にボリュームを作成します。link:hana-fas-nfs-storage-controller-setup.html#volume-configuration-for-sap-hana-single-host-systems["SAP HANA シングルホストシステムのボリューム構成"]link:hana-fas-nfs-storage-controller-setup.html#volume-configuration-for-sap-hana-multiple-host-systems["SAP HANA マルチホストシステムのボリューム構成。"]
. の説明に従って、必要なボリュームオプションを設定します。link:hana-fas-nfs-storage-controller-setup.html#volume-options["ボリュームのオプション"]
. の説明に従ってNFSv3に必要なオプションを設定するか、の説明に従ってNFSv4に必要なオプションを設定しlink:hana-fas-nfs-storage-controller-setup.html#nfs-configuration-for-nfsv3["NFSv3 用の NFS 設定"]ます。link:hana-fas-nfs-storage-controller-setup.html#nfs-configuration-for-nfsv4["NFSv4 の NFS 設定"]
. の説明に従って、ボリュームをネームスペースにマウントし、エクスポートポリシーを設定します。link:hana-fas-nfs-storage-controller-setup.html#mount-volumes-to-namespace-and-set-export-policies["ボリュームをネームスペースにマウントし、エクスポートポリシーを設定"]
...
image:saphana-fas-nfs_image16.png["入力/出力ダイアログを示す図、または書き込まれた内容を表す図"]
...
image:saphana-fas-nfs_image17.png["入力/出力ダイアログを示す図、または書き込まれた内容を表す図"]
...
NOTE: SAP Local Secure Storeサービス（LSS）を使用する場合、 `/lss/shared/SID`ディレクトリ（LSSのデフォルトのインストールディレクトリ）はローカルディスク上にあります。ディスクベースのレプリケーションを使用した災害復旧設定では、NetAppは、すべてのファイルシステムが中央ストレージ上に存在するように、 `SID_shared`ボリューム内に `/lss/shared/SID`ディレクトリ用のディレクトリを作成することを推奨します。
...
| SID共有 | usr-sap＋共有＋lss-共有 | /usr/sap/SID + /hana/shared + /lss/shared/SID 
|===
...
image:saphana-fas-nfs_image18.png["入力/出力ダイアログを示す図、または書き込まれた内容を表す図"]
...
NOTE: SAP Local Secure Storeサービス（LSS）を使用する場合、 `/lss/shared/SID`ディレクトリはすべてのホストでアクセス可能でなければなりません。NetAppは、 `SID_shared`ボリューム内に `/lss/shared/SID`ディレクトリ用のディレクトリを作成することを推奨します。
...
| SID_shared | lss-shared | /lss/shared/ | すべてのホストにマウントされています 
...
| Snapshot コピーの自動作成を無効にする | vol modify -vserver <vserver-name> -volume <volname> -snapshot-policy none 


| SID_sharedボリュームを除き、ランサムウェア対策を無効にします | vol modify -vserver <vserver-name> -volume <volname> -anti-ransomware-state disabled 
...
NOTE: ID番号を無効にするには、ユーザー管理が必要であることに注意してください（を参照）。link:hana-fas-nfs-install-prep-nfsv4.html["NFSv4 用の SAP HANA インストールの準備"]


NOTE: NFSv4ドメインIDは、すべてのLinuxサーバ（）およびSVMで同じ値に設定する必要があります/`etc/idmapd.conf`（を参照）。link:hana-fas-nfs-install-prep-nfsv4.html["NFSv4 用の SAP HANA インストールの準備"]
