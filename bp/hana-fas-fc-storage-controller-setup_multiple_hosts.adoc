...
次の図は、 4+1 のマルチホスト SAP HANA システムのボリューム構成を示しています。各 SAP HANA ホストのデータボリュームとログボリュームは、異なるストレージコントローラに分散されます。たとえば、ボリューム「 `S ID_data_mnt00001 」はコントローラ A に設定され、ボリューム「 S ID_LOG_mnt00001 」はコントローラ B に設定されています各ボリュームに 1 つの LUN を設定します。
 ...
 `/hana/shared`ボリュームはすべてのHANAホストからアクセス可能である必要があるため、NFSを使用してエクスポートされます。 `/hana/shared`ファイルシステムに特定のパフォーマンスKPIはありませんが、NetAppでは10Gbイーサネット接続の使用を推奨します。
...
NOTE: SAP Local Secure Storeサービス（LSS）を使用する場合、 `/lss/shared/SID`ディレクトリはすべてのホストからアクセス可能である必要があります。NetAppでは、 `/lss/shared/SID`ディレクトリ用のボリュームを作成し、NFSを使用してすべてのホストにマウントすることを推奨しています。
...
LVMボリュームグループのLUNは、異なるアグリゲート内、かつ異なるコントローラに格納する必要があります。次の表は、2+1 SAP HANAマルチホストシステムのボリュームグループあたり2つのLUNの例を示しています。


NOTE: SAP HANA KPI を満たすために LVM を使用して複数の LUN を組み合わせる必要はありませんが、推奨されます。
...
NOTE: 以下の例では、ボリューム作成時にこれらのオプションを設定しています
