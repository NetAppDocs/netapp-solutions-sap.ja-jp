...
. の説明に従って、必要なアグリゲートを作成して構成します。 configuration. 」
. の説明に従って、 Storage Virtual Machine （ SVM ）を作成します。 virtual machine configuration. 」
. の説明に従って、 Logical Interface （ LIF ；論理インターフェイス）を作成します。 interface configuration. 」
. ポートセットを作成します（を参照）。 port sets. 」
. 作成の説明に従って、アグリゲート内にイニシエータグループ、ボリューム、および LUN を作成します。 and volumes and mapping LUNs to initiator groups. 」
