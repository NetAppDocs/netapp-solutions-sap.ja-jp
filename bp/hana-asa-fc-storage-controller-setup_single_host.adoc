...
以下の表は、シングルホストSAP HANAシステムのLUN構成を示しています。

|===
| 目的 | LUN | LVM論理ボリューム 


| データ | FC5_data_mnt00001_1 FC5_data_mnt00001_2 | FC5_data_mnt00001-vol 


| ログ | FC5_log_mnt00001_1 FC5_log_mnt00001_2 | FC5_log_mnt00001-vol 


| 共有 | FC5_shared | 該当なし 
|===

NOTE: 上記の構成では、ユーザSID1admのデフォルトのホームディレクトリが格納されている `/usr/sap/SID1`ディレクトリは、ローカルディスク上にあります。ディスクベースのレプリケーションを使用したディザスタリカバリ設定では、NetAppは、すべてのファイルシステムが中央ストレージ上に配置されるように、 `/usr/sap/SID1`ディレクトリ用の追加のLUNを作成することを推奨しています。


NOTE: SAP Local Secure Storeサービス（LSS）を使用する場合、 `/lss/shared/SID`ディレクトリ（LSSのデフォルトのインストールディレクトリ）は、ローカルディスク上にあります。ディスクベースのレプリケーションを使用した災害復旧設定では、NetAppは、すべてのファイルシステムが中央ストレージ上に存在するように、 `/lss/shared/SID`ディレクトリ用の追加のLUNを作成することを推奨します。
