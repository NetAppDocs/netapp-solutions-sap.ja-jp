...
SnapCenterを使用したSAPシステムリフレッシュ操作の詳細情報については、 https://docs.netapp.com/us-en/netapp-solutions-sap/lifecycle/sc-copy-clone-introduction.html["SnapCenter を使用して SAP HANA システムのコピーおよびクローン処理を自動化"]を参照してください。
...
SnapCenterリリース6.2未満には、考慮する必要がある制限があります。
...
「前提条件」セクションで述べたように、SnapCenterリリース < 6.2 の場合、適切なクリーンアップを開始するには、コマンド「systemctl restart spl」を使用して HANA ホストの SnapCenter spl サービスを再起動する必要があります。
