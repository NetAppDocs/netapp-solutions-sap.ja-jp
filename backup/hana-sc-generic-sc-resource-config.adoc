...
前のトピックで説明したSnapCenterの初期構成 link:hana-sc-generic-initial.html["SnapCenter の初期設定"] HANA データベース リソースの構成には資格情報、ストレージ システム、およびポリシーが必要になるため、最初に実行する必要があります。以下の図は、手順と依存関係をまとめたものです。
