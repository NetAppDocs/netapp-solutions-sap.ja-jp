...
NOTE: 作成するポリシーの SnapMirror ラベルは、 SnapCenter ポリシーの設定で定義されたラベルと一致する必要があります。詳細については、「」を参照してください for daily Snapshot backups with SnapVault replication」
...
.. リソース構成が完了したら、「」の説明に従って、リソース保護の構成を実行します protection configuration」
